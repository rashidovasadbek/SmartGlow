...
namespace SmartGlow.Infrastructure.Common.Caching;
...
public class LazyMemoryCacheBroker(IOptions<CacheSettings> cacheSettings, IAppCache memoryCache) : ICacheBroker
{
    ...
    {
        AbsoluteExpirationRelativeToNow = TimeSpan.FromMinutes(cacheSettings.Value.AbsoluteExpirationInSeconds),
        SlidingExpiration = TimeSpan.FromMinutes(cacheSettings.Value.SlidingExpirationInSeconds)
    };
    ...
    public ValueTask<T?> GetOrSetAsync<T>(string key, Func<T> valueProvider, CacheEntryOptions? entryOptions = default,
        CancellationToken cancellationToken = default)
    {
